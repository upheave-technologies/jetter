...
- **DEC-EM5 — UI is a maintenance-specific inline edit panel, not a reuse of the reservation edit panel.** New container ('use client' slim proxy) + pure panel: quantity 1–6 grid, Od/Do HH:MM time inputs, notes, save/cancel — opened from an edit affordance on each editable maintenance row. NO live availability verdict. _Rejected:_ reusing the reservation edit panel — it renders duration presets, a renter-name field, and a live availability verdict, none of which apply to maintenance. _Why:_ an honest, maintenance-specific surface with Croatian copy consistent with the create panel ("Uredi nedostupnost").
...
- On save, `endTime` must remain after `startTime`, and quantity must stay within 1..6 (`1..FLEET_SIZE`); violations are rejected with a mapped validation error.
...
- [ ] T3 — Build `MaintenanceEditContainer` ('use client' slim proxy) + `MaintenanceEditPanel` (pure): quantity 1–6 grid, Od/Do HH:MM inputs, notes, save/cancel; wire onto each editable maintenance row. No availability verdict. Croatian copy consistent with the create panel ("Uredi nedostupnost"). (owner: frankie)
...
2026-07-03  spec  fleet size 8 → 6 reconciliation (no code change in this change-unit). The maintenance quantity ceiling follows `FLEET_SIZE`, which is now 6 (was 8). Updated the normative references in this SPEC — the AC ("quantity must stay within 1..6"), DEC-EM5 and T3 ("quantity 1–6 grid") — to match. DEC-EM3 already reads `1..FLEET_SIZE` and needed no edit; the constant is the source of truth. The decision of record for the fleet-size change is DEC-P10 in the sibling reservation-pivot SPEC (`system/context/bookings/features/reservation-pivot/SPEC.md`), which owns `FLEET_SIZE` and the DB `bookings_quantity_range` CHECK tighten (1..6). The 2026-07-02 frankie change-log entry above (which mentions the "quantity 1–8 grid") is an append-only historical record of the code as built and is left as-is.
